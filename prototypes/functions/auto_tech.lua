...
local scc = require('luagraphs.connectivity.StronglyConnectedComponents')
require('defines')
...
local starting_items = table.array_to_dictionary({'iron-plate', 'copper-plate', 'burner-mining-drill', 'stone-furnace'}, true)
...
pytech.processed_fluids = {}
pytech.processed_techs = {}
...
function pytech.parse_tech(tech)
    ...
    if pytech.processed_techs[tech.name] then
       ...
       pytech.processed_techs[tech.name] = true
    end

    if not pytech.techs[tech.name] then
        node.ignore_for_dependencies = true
        return node
    end
...
function pytech.parse_recipe(tech_name, recipe, no_crafting)
    ...
    -- if recipe.name == "fluid-unknown-pyvoid-fluid" then
    ...
    if not pytech.recipes[recipe.name] then
        node.ignore_for_dependencies = true
    else
    ...
    for _, res in pairs(recipe_products) do
        if ((res.amount or 0) > 0 or (res.amount_max or 0) > 0) and (not res.probability or res.probability > 0) then
            if res.type == nt_item then
                ...
                if n_fluid then
                    if not ingredients[res.name] or table.any(ingredients[res.name], function (t)
                        return t ~= (res.temperature or data.raw.fluid[res.name].default_temperature)
                    end) then
...
function pytech.parse_item(item)
    ...
    if not pytech.items[item.name] then
        node.ignore_for_dependencies = true
    else
...
function pytech.parse_fluid(fluid, temperature, fluid_name)
    ...
    if pytech.processed_fluids[name] then
        return node
    else
        pytech.processed_fluids[name] = true
    end

    if not pytech.fluids[fname] then
        node.ignore_for_dependencies = true
    else
        node.ignore_for_dependencies = data.raw.fluid[fname] and data.raw.fluid[fname].ignore_for_dependencies
    end
        ...
        if tech.enabled ~= false and not tech.hidden then
            ...
            if not pytech.is_py_or_base_tech(tech) then
                for _, pre in pairs(tech.prerequisites or {}) do
                    ...
                    if pre_tech and not pytech.is_py_or_base_tech(pre_tech) then
                        if not tech.dependencies then
                            tech.dependencies = {}
                        end
...
function pytech.parse_data_raw()
    ...
    -- Starter items
    for item_name, _ in pairs(starting_items) do
        local item =  pytech.get_prototype('item', item_name, true)

        if item then
            local recipe = {
                name = recipe_start .. item_name,
                ingredients = {},
                results = {{ type = nt_item, name = item_name, amount = 1}}
            }
    ...
    if not table.any(entity.flags or {}, function(v) return v == 'hidden' end) or starting_entities[entity.name] then
        ...
        if entity.type == 'boiler' then
            ...
            if (entity.mode or 'heat-water-inside') == 'output-to-separate-pipe' then
                ...
                filter = entity.fluid_box.filter or entity.fluid_box.filter
                temp = data.raw.fluid[filter].max_temperature
            end

            if filter then
                pytech.insert_double_lookup(pytech.fluids, filter, temp)
            else
                error('ERROR: Unsupported feature: Unfiltered boiler')
            end
